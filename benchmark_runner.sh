...
ITERATIONS, OUTPUT_DIR, PYTHON_BIN.
EOF
}
...
discover_java_targets() {
  [[ -d "$JAVA_DIR" ]] || return 0
  local class_file rel fqcn has_main
  while IFS= read -r -d '' class_file; do
    rel="${class_file#"$JAVA_DIR"/}"
    rel="${rel%.class}"
    fqcn="${rel//\//.}"
    [[ "$fqcn" == *'$'* ]] && continue
    has_main=$(javap -public -classpath "$JAVA_CLASSPATH" "$fqcn" 2>/dev/null \
               | grep -c 'public static void main(java\.lang\.String\[\])')
